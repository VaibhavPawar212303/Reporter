...
export function CoverageGap({ selectedBuild, masterCases, filterStatus }: any) {
  // 🔥 FIX: Calculate the missing codes specifically for this component
  const missing = useMemo(() => {
    if (!selectedBuild || !masterCases || masterCases.length === 0) return [];
    
    // 1. Collect all codes found in the current automated build
    ...
    // 2. Return master cases that ARE NOT in that set
    return masterCases.filter((mc: any) => !automatedCodes.has(mc.caseCode));
  }, [selectedBuild, masterCases]);

  // Only show the gap if we are in 'all' filter mode and there are missing cases
  ...
  return (
    <section className="mt-32 pt-20 border-t border-white/5 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-2xl font-black text-white tracking-tight italic">Coverage Gap Analysis</h2>
          <p className="text-zinc-500 text-sm font-medium">Master requirements not yet detected in automated scripts</p>
        </div>
      </div>
        ...
        {missing.map((mc: any) => (
          <div key={mc.caseCode} className="bg-[#0c0c0e]/50 border border-dashed border-white/10 rounded-[2rem] p-6 hover:border-indigo-500/20 transition-all group">
            <div className="flex justify-between items-start mb-4">
              <span className="text-[10px] font-black text-zinc-500 bg-white/5 px-2 py-1 rounded-md border border-white/5 group-hover:text-indigo-400 transition-colors">
                {mc.caseCode}
              </span>
